...
const PlanItem: React.FC<PropTypes> = ({ product, active }) => {
  return (
    <li>
      <Link
        className="flex items-center rounded bg-white px-6 py-5 text-slate-800"
        href={`/checkout?plan=${product.price.lookup_key}`}
        shallow
      >
        <span className={cn(active ? 'bg-primary-600' : 'border bg-gray-100', 'mr-3 flex h-5 w-5 rounded-full')}>
          {active && <Check strokeWidth="4" className="m-auto block h-3 w-3 text-white" />}
        </span>
        <span className="block flex-1 text-lg">{product.name}</span>
